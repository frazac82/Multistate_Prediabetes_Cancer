...
cls
clear all
global off "...\Database_msm"

cap use "$off/predmdb2.dta", clear
tab type gender, m
drop smk_date weight w_date monthday pracid yob
order patid age_index gender bmi imd2010_5 ethnic white smk_status 	/*
	  */dob index 													/*
	  */t2d_date t2d_outcome t2d_censoring							/*
	  */cancersite cancer_date cancer cancer_censoring 				/*
	  */cause dodeath death death_censoring
renames age_index index cancersite cancer cause dodeath death smk_status \ /*
*/	ageind indexd cancer_site cancer_outcome death_cause death_date death_outcome smk
...
***************************************************************
sum death_outcome if death_date == .
sum death_outcome if death_date != .
...
list patid indexd t2d t2di if t2d == 0
list patid indexd cancer canceri if cancer == 0
list patid indexd surv survi if surv == 0
*********************************************************			
...
*t2d vs cancer*
gen ddc = t2d - cancer
drop if ddc == 0 & t2di == 1 & canceri == 1				/*525 individuals DROPPED*/
...
replace t2d = surv - (0.5/365.24) if dds == 0 & t2di == 1 & survi == 1			/*27 people t2d=death*/
drop dds

count if dcs == 0 & canceri == 1 & survi == 1									/*72 people cancer=death*/
...
replace t2d = surv if ddc>0 & t2di == 1 & canceri == 1	/*Not interested in T2DM after cancer*/
...
tabstat bmi, statistics(N min max) by(bmig)
tab bmig, nolab
...
tabstat ageind, statistics(N min max) by(ageg)
tab ageg, nolab
...
tabstat ageind, statistics(N min max) by(ageg)
groups gender ageg, missing sepby(gender)
...
cap save "$off/multistate_structure.dta", replace 
...
cap save "$off/pseudocounts_structure.dta", replace  
...
clear
use "$off/pseudocounts_structure.dta", clear
...
save "$off/ps_probs2.dta", replace
...
*---------------------------------------------*
***Probs - OVERALL POPULATION-----------------*
*---------------------------------------------*
cls
clear all
global off "...\Database_msm"
cap use "$off/ps_probs2.dta", replace
...
local files: dir "...\Results_msm/" files "po_g*.dta"
foreach file in `files' {
	qui use "...\Results_msm/`file'", clear
	qui gen filen = "`file'"
...
save "$off/States_Overall_probs2.dta", replace

local files: dir "...\Results_msm/" files "po_g*.dta"
foreach file in `files' {
	qui erase "...\Results_msm/`file'"
}


*---------------------------------------------*
***LOS - OVERALL POPULATION-------------------*
*---------------------------------------------*
cls
clear all
global off "...\Database_msm"
cap use "$off/ps_los2.dta", replace
...
local files: dir "...\Results_msm/" files "lo_g*.dta"
foreach file in `files' {
	qui use "...\Results_msm/`file'", clear
	qui gen filen = "`file'"
...
tab time state if sex == 1
tab time state if sex == 2
save "$off/States_Overall_los2.dta", replace
...
*--------------------------------------------*
***Probs - RELATIVE [DEATH COMBINED] on HPC--*
*--------------------------------------------*
cls
clear all
global hpc ".../Database_msm"
use "$hpc/ps_probs2.dta", clear
...
compress
save "$hpc/States_Relative_prob2.dta", replace


*------------------------------------------*
***LOS - RELATIVE [DEATH COMBINED] on HPC--*
*------------------------------------------*
cls
clear all
global hpc ".../Database_msm"
use "$hpc/ps_los2.dta", clear
...
qui sencode parm, gen(sparm)
qui sort sex state sparm
save "$hpc/States_Relative_los2.dta", replace


******************************************************************
**#GLM estimates from pseudocounts -- DEATH***********************
******************************************************************

*---------------------------------*
***Probs absorbing OVERALL on HPC-*
*---------------------------------*
cls
clear all
global hpc ".../Database_msm"
cap use "$hpc/ps_probs2.dta", replace
...
drop if nmiss != 0
drop nmiss
keep patid gender ps_p*_10 ageg
gen ps_pdeath_10 = ps_p4_10 + ps_p8_10 + ps_p7_10 + ps_p3_10

cd ".../Results_msm"
forval g = 1/2 {
	qui glm ps_pdeath_10 ib(frequent).ageg if gender == `g', eform link(log) vce(robust)
	qui cap margins ageg, saving("oadp_g`g'", replace)
	di "Out = prob_death_overall | Sex == `g' | Time == 10 || $S_TIME"
}

qui use ".../Results_msm/oadp_g1", clear
qui gen sex = "Men"
qui append using ".../Results_msm/oadp_g2"
qui replace sex = "Women" if sex == ""
keep _margin _ci_lb _ci_ub _m1 sex
renames _margin _ci_lb _ci_ub _m1 \ prob min95 max95 ageg
order sex ageg, first
foreach var of varlist prob-max95 {
	replace `var' = `var'*100
}
export excel using ".../Results_msm/Death_combined.xls", sheet("Probs_Death", modify) firstrow(variables) keepcellfmt
qui erase ".../Results_msm/oadp_g1.dta"
qui erase ".../Results_msm/oadp_g2.dta"


*-------------------------------------------*
***LOS nonabsorbing (RMST) - OVERALL on HPC-*
*-------------------------------------------*
cls
clear all
global hpc ".../Database_msm"
cap use "$hpc/ps_los2.dta", replace
...
drop if nmiss != 0
drop nmiss
keep patid gender ps_los*_10 ageg	
gen ps_rmst_10 = ps_los1_10 + ps_los5_10 + ps_los6_10 + ps_los2_10

cd ".../Results_msm"
forval g = 1/2 {
	qui glm ps_rmst_10 ib(frequent).ageg if gender == `g', link(identity) vce(robust)
	qui cap margins ageg, saving("oadl_g`g'", replace)
	di "Out = RMST_overall | Sex == `g' | Time == 10 || $S_TIME"
}

qui use ".../Results_msm/oadl_g1", clear
qui gen sex = "Men"
qui append using ".../Results_msm/oadl_g2"
qui replace sex = "Women" if sex == ""
keep _margin _ci_lb _ci_ub _m1 sex
renames _margin _ci_lb _ci_ub _m1 \ los min95 max95 ageg
order sex ageg, first
export excel using ".../Results_msm/Death_combined.xls", sheet("LOS_Death", modify) firstrow(variables) keepcellfmt
qui erase ".../Results_msm/oadl_g1.dta"
qui erase ".../Results_msm/oadl_g2.dta"
...
******************************************************************
frame reset
use "...\Database_msm/multistate_structure.dta", replace 
...
gsort -sex tt
drop _merge
save "...\Database_msm/Rates_PreD_T2DM.dta", replace


*####################################################################################################################################*
**************************************************************************************************************************************
**#OUTPUT -- GRAPHS -- TABLES -- IN OFFICE********************************************************************************************
**************************************************************************************************************************************
*####################################################################################################################################*
...
**#Table-1 BASELINE------*
*------------------------*
use "...\Database_msm\db2.dta", replace
...
*/  exportexcel("...\Results_msm\Table1_baseline", replace)

sum surv, d
total surv
distplot surv, over(gender)
graph close _all
...
graph save "Graph" "...\Results_msm\Figure1_TraM.gph", replace
restore
...
msboxes, transmatrix(tmat) id(patid)                       								///
	...
	transnames(T1 T2 T3 T4 T5 T6 T7)													///
	boxheight(0.10) ysize(4) xsize(8) 	
graph save "Graph" "...\Results_msm\Figure1_TraW.gph", replace
restore

graph combine "...\Results_msm\Figure1_TraW.gph" "...\Results_msm\Figure1_TraM.gph"	///
	  , scale(0.7) xsize(9) ysize(5) imargin(small) nocopies
graph save "Graph" "...\Results_msm\Figure1_Transitions.gph", replace
graph close _all
qui erase "...\Results_msm\Figure1_TraM.gph"
qui erase "...\Results_msm\Figure1_TraW.gph"


*---------------------------------------*
**#Figure-2 PROBS CUMULATIVE------------*
*---------------------------------------*
use "...\Database_msm\States_Overall_probs2.dta", clear
...
twoway (area  c1    time, sort fcolor(%50) fintensity(35) lwidth(none) yaxis(2)) 	///
	   ...
	   xtitle(`"Time (years)"') xlabel(0(1)10) xmtick(##2) 									 				 		///
...
graph save "Fig2" "...\Results_msm\Figure2_CumPROBS.gph", replace
rename est_? state_?
export excel using "...\Results_msm\Aggregate_results.xls", sheet("Figure_2", modify) firstrow(variables) keepcellfmt
  

*------------------------------------*
**#Figure-3 LOS CUMULATIVE-----------*
*------------------------------------*
use "...\Database_msm\States_Overall_los2.dta", clear
...
graph save "Fig3" "...\Results_msm\Figure3_CumLOS.gph", replace
drop min95_* max95_*
rename est_? state_?   
export excel using "...\Results_msm\Aggregate_results.xls", sheet("Figure_3", modify) firstrow(variables) keepcellfmt
...
graph save "Fig4" "...\Results_msm\Figure4_ProbsLOS.gph", replace
export excel using "...\Results_msm\Aggregate_results.xls", sheet("Figure_4", modify) firstrow(variables) keepcellfmt
...
*/  exportexcel("...\Results_msm\TableS1_basAgeg", replace)
...
forvalues k = 1/7 {
	qui strate gender if _trans`k' == 1, per(1000) output("...\Results_msm\rates`k'_sex", replace)
}
...
forvalues k = 1/7 {
	qui use "...\Results_msm\rates`k'_sex", clear
...
export excel using "...\Results_msm\TableS2_rates.xls", firstrow(variables) replace

forvalues k = 1/7 {
	qui erase "...\Results_msm\rates`k'_sex.dta"
}


*----------------------------------------------*
**#Table-S3 PREDICTED PROPORTIONS OF DEATHS----*
*----------------------------------------------*
use "...\Database_msm\States_Overall_probs2.dta", clear
drop se_prob filen metric
order sex ageg state time  
...
export excel using "...\Results_msm\TableS3_propdeath.xls", firstrow(variables) replace
...
*------------------------------------*
use "...\Database_msm\pseudocounts_structure.dta", replace 
net install sankey, from("https://raw.githubusercontent.com/asjadnaqvi/stata-sankey/main/installation/") replace
which sankey /*v 1.61 -- 22 jul 2023 -- https://github.com/asjadnaqvi/stata-sankey*/
...
graph save "Sankey" "...\Results_msm\FigureS2_Sankey.gph", replace
graph close _all
...
*-----------------------------------*
use "...\Database_msm\Rates_PreD_T2DM.dta", clear
sencode sex, replace
...
graph combine FigS3r FigS3d, xcommon cols(1) xsize(6) ysize(6) scale(0.9) name("FigS3", replace)
graph save "FigS3" "...\Results_msm\FigureS3_RatesPreDT2DM.gph", replace
export excel using "...\Results_msm\Aggregate_results.xls", sheet("FigS3", modify) firstrow(variables) keepcellfmt
...
foreach m in ageg bmig smkn imd2010_5 ethnic {
	qui cap use "...\Results_msm\Events_`m'", clear
...
graph save "FigS4" "...\Results_msm\FigureS4_StratifiedEvents.gph", replace
export excel using "...\Results_msm\Aggregate_results.xls", sheet("FigS4", modify) firstrow(variables) keepcellfmt

foreach m in ageg bmig smkn imd2010_5 ethnic {
	qui cap erase "...\Results_msm\Events_`m'.dta"
}


*------------------------------------------*
**#Figure-S5 Risk Ratio Forest Probs-------*
*------------------------------------------*
use "...\Database_msm\States_Relative_prob2.dta", clear
...
foreach n in Women Men {
	forvalues s = 1/5 {
		...
		qui forestplot estimate min95 max95 if sex == "`n'" & state == `s'										///
			, eform effect("") lcols(plotidn parm)                        	 									///
			...
			spacing(4.5) yline(2.5 7.5 10.5 15.5, lwidth(vthin) lpattern(vshortdash)) 							///
...
graph combine PWomen_1 PWomen_2 PWomen_3 PWomen_4 PWomen_5 PMen_1 PMen_2 PMen_3 PMen_4 PMen_5, nocopies ycommon rows(2) name("FigS5", replace) nodraw
graph save "FigS5" "...\Results_msm\FigureS5_RelPROBS.gph", replace
drop plotidn _EFFECT stderr
order sex plotid state parm
foreach var of varlist estimate-max95 {
	replace `var' = exp(`var')
}
export excel using "...\Results_msm\Aggregate_results.xls", sheet("FigS5", modify) firstrow(variables) keepcellfmt


*------------------------------------------*
**#Figure-S6 Forest LOS difference---------*
*------------------------------------------*
use "...\Database_msm\States_Relative_los2.dta", clear
drop eq npart time out sparm
order sex state, first
split parm, p(".")
drop parm1
...
graph combine LWomen_1 LWomen_2 LWomen_3 LWomen_4 LWomen_5 LMen_1 LMen_2 LMen_3 LMen_4 LMen_5, nocopies ycommon rows(2) name("FigS6", replace) nodraw
graph save "FigS6" "...\Results_msm\FigureS6_RelLOS.gph", replace
drop plotidn _EFFECT stderr
order sex plotid state parm
export excel using "...\Results_msm\Aggregate_results.xls", sheet("FigS6", modify) firstrow(variables) keepcellfmt
graph close _all
clear all